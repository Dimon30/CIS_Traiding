...
try {
  ...
  assert(initialMonth.toLowerCase().includes("октябрь 2025"), "По умолчанию должен открываться октябрь 2025")
  assert((await signalCalendar.innerText()).includes("3 сигналов в этом месяце"), "В открытом месяце должно быть 3 сигнала")
  assert((await page.locator('[data-signal-day="true"]').allInnerTexts()).join(",") === "4,9,22", "Октябрьские даты не совпадают с демо-расписанием")
  ...
  assert((await page.locator('[data-signal-day="true"]').allInnerTexts()).join(",") === "2,19", "Сентябрьские даты не совпадают с демо-расписанием")
  ...
  await nextMonthButton.click()
  assert((await signalCalendar.innerText()).includes("2 сигналов в этом месяце"), "В следующем месяце должно быть 2 сигнала")
  assert((await page.locator('[data-signal-day="true"]').allInnerTexts()).join(",") === "6,18", "Ноябрьские даты не совпадают с демо-расписанием")
  assert(await nextMonthButton.isDisabled(), "Календарь содержит больше трёх месяцев")
  await previousMonthButton.click()
  ...
  await page.waitForTimeout(650)
  assert((await page.getByText(expectedSignalText, { exact: false }).count()) >= 1, "Текст сигнала не найден в плашке")
