...
коду коридора. Интерфейс формирует демонстрационный текст локально, по выбранному
дню календаря. Тексты намеренно не входят в schema v1: это два UX-сценария,
сравнение со средним за месяц и улучшение курса четвёртый день подряд. Их
процентные значения являются демонстрационными, а не полями ML-экспорта. Плашка
показывается только для свежего пуша; просроченный пуш можно открыть, но он не
подтверждает актуальность курса.
