...
class WeeklyPolicyTest(unittest.TestCase):
    ...
    def test_cooldown_survives_year_threshold_inactive_and_is_isolated(self):
        ...
        state.apply(first, SelectedPolicy(.8, "active"), "main", 4)
        state.apply(self.frame(["2025-01-01"]), SelectedPolicy(None, "inactive"), "main", 4)
        next_frame = self.frame(["2025-01-02", "2025-01-04"])
        result = state.apply(next_frame, SelectedPolicy(.1, "active"), "main", 4)
        self.assertEqual(result["selected_signal"].tolist(), [False, True])
        self.assertTrue(state.apply(next_frame.head(1), SelectedPolicy(.1, "active"), "frontier", 4)["selected_signal"].iloc[0])
        self.assertTrue(state.apply(next_frame.head(1).assign(corridor="RUB_KZT"), SelectedPolicy(.1, "active"), "main", 4)["selected_signal"].iloc[0])
    ...
    def test_exact_sampler_matches_exhaustive_support_and_draw_isolation(self):
        ...
        draws = continuous_draws(frame, schedule, mode="calendar_month", draws=800, seed=42, cooldown_days=4, stream_id="test")
        feasible = set()
        for a, b in itertools.product([0, 1], [2, 3, 4]):
            if (frame.iloc[b]["date"] - frame.iloc[a]["date"]).days > 4:
        ...
        first = continuous_draws(frame, schedule, mode="calendar_month", draws=1, seed=42, cooldown_days=4, stream_id="test")
        pd.testing.assert_frame_equal(first.reset_index(drop=True), draws.loc[draws["draw_id"].eq(0)].reset_index(drop=True))
        audit_continuous_schedules(schedule, draws, 4)

    def test_single_schedule_weekday_quotas_and_infeasible(self):
        frame = self.frame(["2024-12-30", "2025-01-02", "2025-01-06"])
        schedule = frame.iloc[[0, 2]]
        draws = continuous_draws(frame, schedule, mode="calendar_month_weekday", draws=3, seed=4, cooldown_days=4, stream_id="single")
        ...
        with self.assertRaisesRegex(ValueError, "No cooldown-feasible"):
            continuous_draws(frame.head(2), frame.head(2), mode="calendar_month", draws=1, seed=4, cooldown_days=4, stream_id="bad")
    ...
    def test_quality_full_exposure_fallback_and_tie_break(self):
        ...
        config = {"cooldown_days": 4, "minimum_policy_signals": 12, "minimum_safety": .75,
    ...
    def test_continuous_cadence_and_audit(self):
        ...
        with self.assertRaisesRegex(AssertionError, "Continuous cooldown"):
            audit_continuous_schedules(bad, bad.assign(draw_id=0), 4)
